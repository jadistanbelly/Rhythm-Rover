...
async def download_and_store_audio(video_url: str, start: str, end: str, user_id: str):
    """Download a user's outro audio and store it in user_audio_files."""
    try:
        ...
        safe_remove_audio_file(user_audio_files[user_id][1], Path)
        user_audio_files[user_id][1] = audio_path
        ...
        logger.exception("Failed to store outro audio")
        return "Failed to download outro audio. Check the URL and timestamps, then try again."
...
async def outro(interaction: discord.Interaction, video_url: str, start: str, end: str):
    """Register outro audio for the current user."""
